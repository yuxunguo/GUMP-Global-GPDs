...
def dsigma_Jpsi_dt(y: float, xB: float, t: float, Q: float, phi: float, HTFF_jpsi: complex, ETFF_jpsi: complex):
    ...
    return LplusT * gevtonb * 4 * np.pi ** 2 * alphaEM * xB ** 2 / ((Q**2 + M_jpsi**2) ** 2)  * (Real(HTFF_jpsi* Conjugate(HTFF_jpsi)) - t/4/ M_p**2 * Real(ETFF_jpsi* Conjugate(ETFF_jpsi))) * (Q/ (Q**2 + M_jpsi**2)) ** 2
