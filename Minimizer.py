...
NumofGPDSpecies = 4
# Number of flavors, 3 flavors are needed including gluon, flavor singlet (u+d), and flaver non-singlet (u-d)
Numofflavor = 3 
...
xi2_Factor = 2
# A factor of 2 for separate sea and valence distributions 
Sea_Val_Factor = 2
# Total number of parameters 
Tot_param_Size = NumofGPDSpecies * xi2_Factor * Numofflavor * Sea_Val_Factor *  init_NumofAnsatz * Single_Param_Size

"""
The parameters will form a 6-dimensional matrix such that each para[#1,#2,#3,#4,#5,#6] is a real number.
#1 = [0,1,2,3] corresponds to [H, E, Ht, Et]
#2 = [0,1] corresponds to [xi^0 terms, xi^2 terms]
#3 = [0,1,2] corresponds to [NS, S, G] in the flavor space
#4 = [0,1] corresponds to [Sea, Valence] for quarks, for gluons the valence ones should always be turned off
#5 = [0,1,...,init_NumofAnsatz] corresponds to different set of parameters
#6 = [0,1,2,3] correspond to [norm, alpha, beta, alphap] as a set of parameters
"""
...
def ParaConvert(Allparameter: np.array):
    ...
    nDParam = np.reshape(Allparameter, (NumofGPDSpecies, xi2_Factor, Numofflavor, Sea_Val_Factor, init_NumofAnsatz, Single_Param_Size))
    return nDParam
...
Init_Para_NS = [Init_Para_NS_Sea, Init_Para_NS_Valence]
