...
We have three main callables: ``DVCS_xsec.dsigma_DVCS()``, ``DVCS_xsec.dsigma_INT()`` and ``DVCS_xsec.dsigma_BH()``,
...
Besides the three main callables, there is also ``DVCS_xsec.dsigma_TOT()`` that takes the same input and returns the sum of the three functions above.
And the ``DVCS_xsec.dsigma_DVCS_HERA()`` that return the virtual-photon-proton cross-sections and integrate over :math:`\phi`, as measured by HERA. The input :math:`\phi` will not be needed in this case.